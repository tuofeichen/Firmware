...
void BlockLocalPositionEstimator::visionInit()
{
	...
	if (_visionStats.getCount() > REQ_VISION_INIT_COUNT) {
		...
		_visionHome(0) = _x(X_x); // note down most recent position as home ??
		_visionHome(1) = _x(X_y);
		_visionHome(2) = _x(X_z);

		//_visionStats.getMean();
...
void BlockLocalPositionEstimator::visionCorrect()
{
	...
	// y -= _visionHome; no relative to home measurement
