...
void BlockLocalPositionEstimator::flowInit()
{
	...
	if (_flowQStats.getCount() > REQ_FLOW_INIT_COUNT) {
	  ...
	  flow_last_invalid = REQ_FLOW_INIT_COUNT; // avoid flow state jump
...
int BlockLocalPositionEstimator::flowMeasure(Vector<float, n_y_flow> &y)
{
	...
	if (_lidarInitialized && (_lidarFault < fault_lvl_disable)) {
		d = _sub_lidar->get().current_distance
		    + (_lidar_z_offset.get() - _flow_z_offset.get()); // should use agl() instead?
	...
	// mavlink_and_console_log_info(&mavlink_log_pub, "[lpe] flow timespan %5.2f", double(dt_flow));
	y(Y_flow_x) = delta_n(0)/dt_flow;
	y(Y_flow_y) = delta_n(1)/dt_flow;
...
#ifdef CORRECT_POSITION
	C(Y_flow_x, X_x) = 1; // correct position
	C(Y_flow_y, X_y) = 1;
#else
	C(Y_flow_x, X_vx) = 1; // correct velocity
	C(Y_flow_y, X_vy) = 1;
#endif
